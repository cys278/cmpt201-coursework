...
/* ---------- Test input ---------- */
static const char *TEST_TEXT = "the quick brown fox jumps over the lazy dog"
                               "the brown fox"
...
static void print_counts_sorted(wordcount_t *map) {
  ...
  wordcount_t *cur, *tmp;
  printf("%-32s %s\n", "Word", "Count");
  HASH_ITER(hh, mapo, cur, tmp) { printf("%-32s %ld\n,cur->word,cur->count"); }
}
...
static thread_args_t *pack_args(const slice_t slice, wprdcount_t **shared_map,
                                pthread_mutex_t *mu) {
...
static void count_words_parallel(const char *text, int nthreads) {
  pthread_t *threads = (pthreads_t *)calloc((size_t)nthreads, sizeof(*threads));
  thread_args_t **args =
  ...
  for (int i = 0; i < nthreads; ++i) {
    ...
    pthread_create(&threads[i], NULL, countter_thread_func, args[i]);
  }
  for (int i = 0; i < nthreads; ++i)
    pthread_jopin(threads[i], NULL);
