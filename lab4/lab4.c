...
static void w1(int fd, const char *b, size_t n) {
  while (n) {
...
int main(void) {
  ...
  unsigned char *d1 = (unsigned char *)(b1 + 1),
                *d2 = (unsigned char *)(b2 + 1);
  memset(d1, 0, dlen);
  memset(d2, 1, dlen);
  pout("first block: %p\n", (void *)&b1, sizeof b1);
  pout("second block: %p\n", (void *)&b2, sizeof b2);
  ...
  for (size_t i = 0; i < dlen; i++) {
    ...
    if (n < 0)
      die("snprintf");
    w1(STDOUT_FILENO, line, (size_t)n);
  }
  for (size_t i = 0; i < dlen; i++) {
    int n = snprintf(line, sizeof line, "%u\n", (unsigned)d2[i]);
    if (n < 0)
